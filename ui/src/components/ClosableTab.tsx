...
export function ClosableTab({
  ...
  shimmer?: boolean;
  /** Opened from a chat chip and not yet committed to: italic, and the next
   * chip click replaces it. */
  preview?: boolean;
  onSelect: () => void;
  /** Double-click commits the tab, so the next chip click opens beside it. */
  onPromote?: () => void;
  onClose: () => void;
}) {
  return (
    <button
      ...
      onKeyDown={(e) => {
        if (active && e.key === "Enter") onPromote?.();
      }}
      title={preview ? `${label} (double-click or Enter to keep open)` : label}
